...
% def initialize_median(m, first_frame):
%     h, w = first_frame.shape[:2]
%     med_data = np.dstack([first_frame]*m)
%     med_img = first_frame.copy()
%     cum_img = np.zeros((h, w, 256))
%     # that's just fucking numpy magic
%     np.put_along_axis(cum_img, med_img[..., None], values=m, axis=2)
%     cum_img = np.cumsum(cum_img, axis = 2)
%     return med_data, med_img, cum_img
\begin{algorithm}[H]
...
% @njit
% def update_cum_img_by_img(cum_img, increment_img, decrement_img):
%     h,w = increment_img.shape[:2]
%     for i in range(h):
%         for j in range(w):
%             cum_img[i,j,increment_img[i,j]]+=1
%             cum_img[i,j,decrement_img[i,j]]-=1
%     return cum_img
% Valeriy Krygin, [23.05.22 22:24]
% Касательно алгоритма 3. Смотри. 
% Когда добавляешь в список изображение, то нужно ведь обновить интегральную сумму. 
% А она содержит в себе сумму всех предыдущих цветов. А это значит, что при добавлении 
% пикселя с цветом 200 нужно добавить единичку и в тот элемент интегральной суммы, что 
% соответствует 201, и 202, и так далее до 255. Понимаешь?
% Аналогично с декрементом
% Valeriy Krygin, [23.05.22 22:25]
% То есть, нужен ещё один цикл, мне кажется. Но его сложность константная, потому что, как ты указал в конце, количество яркостей известно
...
% @njit
% def update_med_img(cum_img, med_img, med_data, m):
%     h,w,n_colors = cum_img.shape
%     med_position = int(m/2)
%     for i in range(h):
%         for j in range(w):
%             if cum_img[i,j, med_img[i,j]] >= med_position:
%                 new_med_pos = cum_img[i,j, med_img[i,j]]
%                 for k in range(med_img[i,j] - 1, 0, -1):
%                     if cum_img[i,j, k] <= med_position:
%                         new_med_pos = cum_img[i,j, k+1]
%                         break;
%             else:

%                 for k in range(med_img[i,j] + 1,n_colors, 1):
%                     if cum_img[i,j, k] >= med_position:
%                         new_med_pos = cum_img[i,j, k-1]
%                         break;
%             med_img[i,j] = med_data[i,j,int(new_med_pos-1)]
%     return med_img
\begin{algorithm}[H]
...
% m = 20
% med_data, med_img, cum_img = initialize_median(m, frames[0])
% q_head = 0
% meds = []
% for i in range(1,frames.shape[0]):
%     cum_img = update_cum_img_by_img(cum_img, 
%                                     increment_img = frames[i], 
%                                     decrement_img = frames[i-1])

%     med_img = update_med_img(cum_img, med_img, med_data, m = m)
%     med_data[..., q_head] = frames[i]
%     q_head = (q_head + 1)%m
%     meds.append(med_img)
